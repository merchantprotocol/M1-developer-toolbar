...
class MP_Debug_Model_Observer
{
    ...
    public function getRequestInfo()
    {
        if ($this->requestInfo === null) {
            $this->requestInfo = Mage::getModel('mp_debug/requestInfo');
        }
    ...
    public function startProfiling()
    {
        ...
        $this->canCapture = Mage::helper('mp_debug')->canCapture();
        ...
        if (Mage::helper('mp_debug')->canEnableVarienProfiler()) {
            Varien_Profiler::enable();
        }

        // use customer Zend Db Profiler that also records stack traces
        $stackTraceProfiler = Mage::getModel('mp_debug/db_profiler');
        $stackTraceProfiler->setCaptureStacktraces(Mage::helper('mp_debug')->canEnableSqlStacktrace());
    ...
    public function updateProfiling()
    {
        ...
        // save rendering time
        $requestInfo->setRenderingTime(MP_Debug_Model_Block::getTotalRenderingTime());
    ...
    {
        if (!$this->canCollect() || !Mage::helper('mp_debug')->canPersist()) {
            return;
        }
    ...
    public function onBlockToHtml(Varien_Event_Observer $observer)
    {
        ...
        $blockName = Mage::helper('mp_debug')->getBlockName($block);
    ...
    public function onWebsiteRestriction(Varien_Event_Observer $observer)
    {
        ...
        $helper = Mage::helper('mp_debug');
        if ($helper->canShowToolbar() && $controller instanceof MP_Debug_Controller_Front_Action) {
